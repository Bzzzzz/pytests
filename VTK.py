...
class GtkGLExtVTKRenderWindowInteractor(gtk.gtkgl.DrawingArea):
    ...
    def OnKeyPress(self, wid, event):
        ...
        print 'm: ',m
        ctrl, shift = self._GetCtrlShift(event)
        print 'ctr, shift: ',ctrl,shift
        keycode, keysym = event.keyval, event.string
        print 'keycode ',keycode
        print 'keysym ',keysym
        key = chr(0)
        keyIgnore=[ord('z'),ord('c')]
        if keycode < 256 and keycode not in keyIgnore:
            key = chr(keycode)
            self._Iren.SetEventInformationFlipY(m[0], m[1], ctrl, shift,
                                            key, 0, keysym)
        elif keycode in [65361, 65362, 65363, 65364]:
        #left, top, right, down
            print 'CameraMoving'
            try:
                self.CameraMove(keycode)
            except: pass 

        elif keycode in keyIgnore:
            print 'CameraRotate'
            try:
                self.CameraRotate(keycode)
            except: pass 
